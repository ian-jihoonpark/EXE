...
from transformers import PreTrainedModel, OFAConfig
from transformers.models.ofa.resnet import ResNet
from transformers.modeling_outputs import Seq2SeqModelOutput, Seq2SeqLMOutput, BaseModelOutputWithPastAndCrossAttentions
...
class foreground_detection_hiddenstates(nn.Module):
    r"""
    ...
    def __init__(self):
        super().__init__()
        self.maxpool = nn.MaxPool2d(kernel_size=(2,2), stride=2, padding = 1, return_indices=True),
        self.gelu = nn.GELU(),
        self.maxunpool = nn.MaxUnpool2d(kernel_size=(2,2), stride=2, padding = 1)

    def forward(self, x):
        residual = x
        text_output = x[:,196:,:]
        image_output = x[:,:196,:].reshape(-1,14,14,768)
        image_output= image_output.permute(0,3,1,2)
        
        output,indices = self.maxpool[0](image_output)
        output = self.gelu[0](output)
        output = self.maxunpool(output, indices)
        output = output.permute(0,2,3,1)
        output = output.reshape(-1,196,768)
        output = torch.cat([output, text_output],dim = 1)
        output = output + residual
        return output
    
class foreground_detection_layer(nn.Module):
    r"""
    Foreground and Background Detection
    """
    ...
    def forward(self, x):
        ...
        mask[:,:196,:196] = 1.0
        x,indices = self.maxpool[0](x*mask)
        x = self.gelu[0](x)
        x = self.maxunpool(input = x, indices = indices, output_size = residual.size()[1:])
        x += residual
        
        return x


class foreground_detection_attention(nn.Module):
    r"""
    Foreground and Background Detection
    """
    ...
    def __init__(self):
        ...
        self.gelu = nn.GELU()
    def forward(self, x):
        rel_image_score = x.clone().detach()
        mask = torch.zeros_like(rel_image_score)
        mask[:,14:196,14:196] = 1.0
        rel_image_score -= torch.mean(rel_image_score * mask)
        rel_image_score += torch.abs(rel_image_score)
        x = self.gelu(x)
        x += rel_image_score
...
class OFAForConditionalGeneration(OFAPreTrainedModel):
    ...
    def __init__(self, config: OFAConfig, align_layer):
        super().__init__(config)
        
        self.padding_idx, self.bos_idx = config.pad_token_id, config.bos_token_id
        shared = nn.Embedding(config.vocab_size, config.d_model, self.padding_idx)
        self.cfg = config
        
        self.encoder = OFAEncoder(config, shared)
        self.decoder = OFADecoder(config, embed_tokens = shared, alignment_layer = align_layer)

        self.lm_head = self.decoder.output_projection
        ...
        self.concentration_hidden = config.concentration_hidden
        if config.concentration_hidden:
            self.f_detection = foreground_detection_hiddenstates()
    ...
    def forward(
                ...
                question_mask: Optional[torch.FloatTensor] = None,
        ...
        if self.concentration_hidden:
            encoder_hidden_states = self.f_detection(encoder_hidden_states)
...
class OFAAttention(nn.Module):
    ...
    def __init__(
        ...
        self.concentration_attn = config.concentration_attn
        
        # # Concentration layer
        if config.concentration_attn:
            self.f_detection = foreground_detection_attention()
            self.f_pool = foreground_detection_layer()
    ...
    def forward(
        ...
        if is_cross_attention and past_key_value is not None:
            ...
            if self.align_layer is not None:
                past_key_value = self.align_layer(past_key_value)
        ...
        # Concentration layer in encoder layer
        if self.concentration_attn:
            if not self.is_decoder:
                attn_probs = self.f_detection(attn_probs)
                attn_probs = self.f_pool(attn_probs)
       
       
                
        attn_output = torch.bmm(attn_probs, value_states) 
        
        self.save_attn(attn_probs.requires_grad_(True))
        ...
        if question_mask is not None:
            ...
            question_mask = question_mask.expand(attn_output.size())
            attn_output = attn_output * question_mask
...
class OFAEncoderLayer(nn.Module):
    ...
    def forward(
        ...
        # if self.concentration_attn:
        #         hidden_states = self.f_detection(hidden_states)
...
class OFADecoderLayer(nn.Module):
    ...
    def __init__(self, config: OFAConfig, drop_path_rate=0.0, align_layer = None):
        super().__init__()
        self.embed_dim = config.d_model
...
class OFADecoder(OFAPreTrainedModel):
    ...
    def __init__(self, config: OFAConfig, embed_tokens: Optional[nn.Embedding] = None, output_projection=None, alignment_layer = None):
        super().__init__(config)
        self.dropout = nn.Dropout(config.dropout)
        ...
        self.layers = nn.ModuleList([OFADecoderLayer(config, align_layer = alignment_layer) for _ in range(config.decoder_layers)])
